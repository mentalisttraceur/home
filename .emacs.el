        ...
        (let* ((suffix (concat "-argument-in-" (symbol-name map)))
            ...
            `(progn
                (defun ,universal (prefix-argument)
                    (interactive "P")
                    (if prefix-argument
                        (funcall (universal-argument-more prefix-argument))
                        (funcall (universal-argument)))
                    (command-execute-in-keymap ,map ,prefix))
                (defun ,digit (prefix-argument)
                    (interactive "P")
                    (funcall (digit-argument prefix-argument))
                    (command-execute-in-keymap ,map ,prefix))
                (defun ,negative (prefix-argument)
                    (interactive "P")
                    (funcall (negative-argument prefix-argument))
                    (command-execute-in-keymap ,map ,prefix))
...
(use-package denote
    ...
    (defun denoted-prefix-set (path prefix)
        (let ((title    (denoted-title-get path))
              (tags     (denoted-tag-get path))
              (datetime (denoted-datetime-get path)))
            (denoted-rename-file path datetime prefix title tags)))
    (defun denoted-prefix-edit (path)
    ...
    (defun task-list (&optional tags)
        ...
        (let ((denote-sort-keywords t))
            (setq tags (denote-keywords-sort tags)))
        (if tags
