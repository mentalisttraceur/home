...
(defun music-append (count)
    (interactive "p")
    (music--open count (music-select) 'next))
(music-define-key "a" 'music-append)
(defun music-append-at-end (count)
    (interactive "p")
    (music--open count (music-select) 'last))
(music-define-key "A" 'music-append-at-end)
(defun music-insert (count)
    (interactive "p")
    (music--open count (music-select) (music-playlist-position)))
(music-define-key "i" 'music-insert)
(defun music-insert-at-beginning (count)
    (interactive "p")
...
(defun music-volume-up (count)
    ...
    (let ((down nil)
        ...
        (when (< count 0)
            ...
            (setq count (- count)))
        (dotimes (_ count)
            (setq volume (mpv-ipc-cycle music--socket "volume" nil down)))
