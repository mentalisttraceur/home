...
(use-package evil
    ...
    (defun universal-argument-in-space-map (prefix-argument)
        (interactive "P")
        (if prefix-argument
    ...
    (defun digit-argument-in-space-map (prefix-argument)
        (interactive "P")
        ...
        (if buffer-file-name
            ...
            (if (and (derived-mode-p 'dired-mode)
                ...
                (user-error "%s is not visiting a file or directory"
                            (buffer-name)))))
    (defun delete-buffer-file-or-directory ()
        (interactive)
    ...
    (defun consult-history-execute (prefix-argument)
        (interactive "P")
        (let* ((consult-history-execute t)
            ...
            (when consult-history-execute
                (funcall run))))
    (defun consult-history-execute-quit ()
        (interactive)
        (setq consult-history-execute nil)
        (vertico-exit))
    (defun consult-history-remove (prefix-argument)
        (interactive "P")
    ...
    (defun consult-history-remove-quit ()
        (interactive)
        (setq consult-history-remove nil)
        (vertico-exit))
    (defun consult-history-change (prefix-argument)
        (interactive "P")
        (let* ((consult-history-change t)
               (old-entry (fixed-consult-history prefix-argument))
               (new-entry (when consult-history-change
                              (read-string "Change history to: " old-entry)))
               (history (car (consult--current-history))))
            (when consult-history-change
                (if histdir
                    (progn
    ...
    (consult-customize consult-history-change :prompt "Change history from: ")
    (evil-declare-not-repeat 'consult-history-change)
    (defun consult-history-change-quit ()
        (interactive)
        (setq consult-history-change nil)
        (vertico-exit))
    ...
    (define-key space-map "vb" (lambda ()
        (interactive)
        (if (vc-root-dir)
    ...
    (defun misc-minibuffer-setup ()
        ...
        (let ((quit (cond
                        ...
                        ((eq this-command 'consult-history-change)
                            'consult-history-change-quit)
