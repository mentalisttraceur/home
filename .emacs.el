...
(use-package dired
    ...
    (defun fixed-dired-revert (dired-revert &rest arguments)
        (let ((column (current-column)))
            (apply dired-revert arguments)
            (move-to-column column)))
    (advice-add 'dired-revert :around 'fixed-dired-revert)
...
(use-package ace-window
    ...
    (defun fixed-aw-select (action)
        (with-advice (('aw-window-list :filter-return 'hack-aw-window-list)
            ...
            (aw-select "" action)))
    (defun fixed-aw--overlay-str (string)
        (if-let ((unpadded-length (string-match-p " +" string))
                 (_ (> unpadded-length 1))
                 (length (length string))
                 (fixed-length (max length (- length (1- unpadded-length)))))
            (substring string 0 fixed-length)
            string))
    (advice-add 'aw--overlay-str :filter-return 'fixed-aw--overlay-str)
...
(use-package denote
    ...
    (defun delete-file-update-dired-buffers (path &rest _)
        (setq path (directory-file-name path))
        (dired-remove-file (expand-file-name path)))
    (advice-add 'delete-file :after 'delete-file-update-dired-buffers)
    (advice-add 'delete-directory :after 'delete-file-update-dired-buffers)
