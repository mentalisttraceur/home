...
(defvar smoother-fill-paragraph--recurse t)
...
(defun smoother-fill-paragraph (prefix-argument)
    ...
    (when (in-paragraph-p t)
        (let* ((fill-column (if prefix-argument
                                ...
                                fill-column))
               (_ (fill-paragraph)))
            (smoother-fill-paragraph-1 (count-lines-paragraph)))))
(defun smoother-fill-paragraph-1 (maximum-lines)
    (let* ((minimum-fill-column (max minimum-fill-column (* fill-column 0.5)))
        ...
        (while (and (>= fill-column minimum-fill-column)
                    ...
                    (= (count-lines-paragraph t) maximum-lines))
        ...
        (fill-paragraph)
        (when (and smoother-fill-paragraph--recurse
                   (> maximum-lines 2))
            (let* ((start (fixed-start-of-paragraph-text t))
                   (end (line-beginning-position 2))
                   (line (buffer-substring start end)))
                (delete-region start end)
                (smoother-fill-paragraph-1 (1- maximum-lines))
                (goto-char start)
                (insert line)))))
...
(evil-define-command shred-evil-paste (register)
    (interactive "<x>")
    :suppress-operator t
...
(shred-define-key "p" 'shred-evil-paste)
(shred-define-key "P" 'shred-evil-paste)
...
(use-package denote
    ...
    (defun delete-file-revert-dired-buffers (filename &rest _)
        (dired-remove-file (expand-file-name filename)))
