...
(use-package evil
    ...
    (when (and android (not termux))
        (defun evil-fancy-input-escaper ()
            (when (member last-command-event '(?⁰ ?¹))
                (delete-char -1)
                (evil-force-normal-state)
                t))
        (setq-default post-text-conversion-hook '(evil-fancy-input-escaper))
        (defun use-raw-input ()
            (set-text-conversion-style nil))
        (defun use-fancy-input ()
            (set-text-conversion-style 'action)
            (add-hook 'post-text-conversion-hook 'evil-fancy-input-escaper))
