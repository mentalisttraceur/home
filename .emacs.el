...
(defun mpv-ipc (command)
    (let* ((message `("command" ,command))
        ...
        (gethash "data" result)))
(defun mpv-ipc-get (property)
    (mpv-ipc `("expand-text" ,(concat "${" property "}"))))
(defun mpv-ipc-get-integer (property)
    (let ((string (mpv-ipc-get property)))
        (when (string-match-p "[^0-9]" string)
            (error "mpv %S not integer: %S" property string))
        (string-to-number string)))
(defun mpv-play (path)
    ...
    (if prefix-argument
        (if (integerp prefix-argument)
            (if (> prefix-argument 0)
                ...
                (let* ((current (mpv-ipc-get-integer "playlist-pos"))
                       (target (+ current prefix-argument)))
                    (when (< target 0)
                        (let ((count (mpv-ipc-get-integer "playlist-count")))
                            (setq target (1- count))))
