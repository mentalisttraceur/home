    ...
    (let* ((table (histdir-history-table history))
        ...
        (when-let (replaced-string (dlist-car (bihash-get (aref table 2) key)))
            (unless (equal replaced-string string)
                (let ((keys (gethash replaced-string calls)))
                    (if-let (keys (delete key keys))
                        (puthash replaced-string keys calls)
                        (remhash replaced-string calls)))))
        (sorted-hash-table-put table key string)
        (unless (member key (gethash string calls))
            (push key (gethash string calls)))
    ...
    (let ((table (histdir-history-table history))
        ...
        (when-let ((first   (aref table 3))
            ...
            (let* ((string (sorted-hash-table-pop table key))
                   (keys   (gethash string calls)))
                (if-let (keys (delete key keys))
                    (puthash string keys calls)
        ...
        (let ((string-directory (concat path "/v1/string/"))
            ...
            (when deduplicate
                (let* ((history (gethash path histdir--histories))
                       (calls (histdir-history-calls history))
                       (other-datetimes (gethash entry calls)))
