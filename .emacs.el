...
(histdir-repl-define-key 'normal "0" 'histdir-repl-beginning-of-line)
(histdir-repl-define-key 'normal "$" 'histdir-repl-end-of-line)
    ...
    (let* ((message `("command" ,command))
        ...
        (let ((ipc-error (gethash "error" result)))
            (unless (equal ipc-error "success")
                (error "mpv IPC error: %s" ipc-error)))
        (gethash "data" result)))
(defun mpv-expand (format)
    (mpv-ipc `("expand-text" ,format)))
(defun mpv-expand-integer (format)
    (let ((string (mpv-expand format)))
        (when (string-match-p "[^0-9]" string)
            (error "not integer: %s" string))
        (string-to-number string)))
...
(defun mpv-play (path prefix-argument)
    ...
    (if prefix-argument
        (if (integerp prefix-argument)
            (if (< prefix-argument 0)
                (let* ((count (mpv-expand-integer "${playlist-count}"))
                       (target (+ count prefix-argument)))
                    (mpv-ipc `("loadfile" ,path "insert-at-play" ,target)))
                (mpv-ipc `("loadfile" ,path "insert-at-play" ,prefix-argument)))
...
(defun mpv-remove (prefix-argument)
    ...
    (if prefix-argument
        (if (integerp prefix-argument)
            (if (< prefix-argument 0)
                (let* ((current (mpv-expand-integer "${playlist-pos}"))
                       (target (+ current prefix-argument)))
                    (when (< target 0)
                        (let ((count (mpv-expand-integer "${playlist-count}")))
                            (setq target (1- count))))
                    (mpv-ipc `("playlist-remove" ,target)))
                (mpv-ipc `("playlist-remove" ,prefix-argument)))
