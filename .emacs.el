...
(defun datetime-parse--loop (string &optional now)
    ...
    (let ((parsed (make-decoded-time))
        ...
        (dolist (word words)
            (uncons previous _ (datetime-parse--bind nil parsed integers))
            (setq previous (datetime-parse--future-bias nil previous now))
            (unpack (parsed previous integers)
                    (datetime-parse--1 word parsed previous integers now)))
...
(defun datetime-parse--1 (word parsed previous integers now)
    (cond
        ((string-match-p "^[0-9]\\{3,\\}$" word)
            (setf (decoded-time-year parsed) (string-to-number word)))
        ((string-match-p "^[0-9]+$" word)
            (setq integers (append integers (list (string-to-number word)))))
        ...
        ((string-match-p "^'[0-9]\\{1,3\\}$" word)
            (setf (decoded-time-year parsed)
                  (+ (string-to-number (substring word 1))
                     datetime-parse-two-digit-year-base)))
        ((string-match-p "^[0-9]+mo" word)
            (setf (decoded-time-month parsed) (string-to-number word)))
        ((string-match-p "^[0-9]+\\(d\\|st\\|nd\\|rd\\|th\\)" word)
            (setf (decoded-time-day parsed) (string-to-number word)))
        ((string-match-p "^[0-9]+h" word)
            (setf (decoded-time-hour parsed) (string-to-number word)))
        ((string-match-p "^[0-9]+am" word)
        ...
        ((string-match-p "^[0-9]+m" word)
            (setf (decoded-time-minute parsed) (string-to-number word)))
        ((string-match-p "^[0-9]+s" word)
            (setf (decoded-time-second parsed) (string-to-number word)))
        ((string-match-p "^[-+][0-9]+y" word)
            (let* ((years (string-to-number word))
                   (delta (make-decoded-time :year years)))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (fixed-decoded-time-add parsed delta))))
        ((string-match-p "^[-+][0-9]+mo" word)
            (let* ((months (string-to-number word))
                   (delta  (make-decoded-time :month months)))
                (uncons parsed integers
                        (datetime-parse--bind 'day parsed integers))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (fixed-decoded-time-add parsed delta))))
        ((string-match-p "^[-+][0-9]+w" word)
            (let* ((days  (* 7 (string-to-number word)))
                   (delta (make-decoded-time :day days)))
                (uncons parsed integers
                        (datetime-parse--bind 'hour parsed integers))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (datetime-parse--floor 'hour parsed))
                (setq previous (datetime-parse--floor 'hour previous))
                (setq previous (fixed-decoded-time-add previous nil))
                (setq parsed (fixed-decoded-time-add parsed delta))))
        ((string-match-p "^[-+][0-9]+d" word)
            (let* ((days  (string-to-number word))
                   (delta (make-decoded-time :day days)))
                (uncons parsed integers
                        (datetime-parse--bind 'hour parsed integers))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                ...
                (setq parsed (fixed-decoded-time-add parsed delta))))
        ((string-match-p "^[-+][0-9]+h" word)
            (let* ((hours (string-to-number word))
                   (delta (make-decoded-time :hour hours)))
                (uncons parsed integers
                        (datetime-parse--bind 'minute parsed integers))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (datetime-parse--floor 'minute parsed))
                (setq previous (datetime-parse--floor 'minute previous))
                (setq previous (fixed-decoded-time-add previous nil))
                (setq parsed (fixed-decoded-time-add parsed delta))))
        ((string-match-p "^[-+][0-9]+m" word)
            (let* ((minutes (string-to-number word))
                   (delta   (make-decoded-time :minute minutes)))
                (uncons parsed integers
                        (datetime-parse--bind 'second parsed integers))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (datetime-parse--floor 'second parsed))
                (setq previous (datetime-parse--floor 'second previous))
                (setq previous (fixed-decoded-time-add previous nil))
                (setq parsed (fixed-decoded-time-add parsed delta))))
        ((string-match-p "^[-+][0-9]+s\\($\\|[^t]\\)" word)
            (let* ((seconds (string-to-number word))
                   (delta   (make-decoded-time :second seconds)))
                (uncons parsed integers
                        (datetime-parse--bind nil parsed integers))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (datetime-parse--floor nil parsed))
                (setq previous (datetime-parse--floor nil previous))
                (setq previous (fixed-decoded-time-add previous nil))
                (setq parsed (fixed-decoded-time-add parsed delta))))
    ...
    (list parsed previous integers))

(defun datetime-parse--bind (slot parsed integers)
    (setq parsed (copy-sequence parsed))
    (unless (eq slot 'month)
        (unless (decoded-time-month parsed)
            (when-let (month (pop integers))
                (datetime--validate-month month)
                (setf (decoded-time-month parsed) month)))
        (unless (eq slot 'day)
            (unless (decoded-time-day parsed)
                (when-let (day (pop integers))
                    (let ((year  (decoded-time-year  parsed))
                          (month (decoded-time-month parsed)))
                        (datetime--validate-day year month day)
                        (setf (decoded-time-day parsed) day))))
            (unless (eq slot 'hour)
                (unless (decoded-time-hour parsed)
                    (when-let (hour (pop integers))
                        (datetime--validate-hour hour)
                        (setf (decoded-time-hour parsed) hour)))
                (unless (eq slot 'minute)
                    (unless (decoded-time-minute parsed)
                        (when-let (minute (pop integers))
                            (datetime--validate-minute minute)
                            (setf (decoded-time-minute parsed) minute)))
                    (unless (eq slot 'second)
                        (unless (decoded-time-second parsed)
                            (when-let (second (pop integers))
                                (datetime--validate-second second)
                                (setf (decoded-time-second parsed)
                                      second))))))))
    (cons parsed integers))
                ...
                (unless (or (eq slot 'day)
                    ...
                    (unless (or (eq slot 'hour)
                        ...
                        (unless (or (eq slot 'minute)
                            ...
                            (when (and (decoded-time-second parsed)
                                       (< (decoded-time-second parsed)
                                          (decoded-time-second now)))
                                (setq delta (make-decoded-time :minute 1))))
                        (when (and (decoded-time-minute parsed)
                                   (< (decoded-time-minute parsed)
                                      (decoded-time-minute now)))
                            (setq-if-nil delta (make-decoded-time :hour 1))))
                    (when (and (decoded-time-hour parsed)
                               (< (decoded-time-hour parsed)
                                  (decoded-time-hour now)))
                        (setq-if-nil delta (make-decoded-time :day 1))))
                (when (and (decoded-time-day parsed)
                           (< (decoded-time-day parsed)
                              (decoded-time-day now)))
                    (setq-if-nil delta (make-decoded-time :month 1))))
            (when (and (decoded-time-month parsed)
                       (< (decoded-time-month parsed)
                          (decoded-time-month now)))
                (setq-if-nil delta (make-decoded-time :year 1))))
...
(defun datetime-read--preview (now cell)
    (when (eq (current-buffer) (car cell))
        (let ((input (minibuffer-contents)))
            (condition-case error
                (let-unpack ((final prior) (datetime-parse--loop input now)
                             (_ _ _ day month year) final)
                    ...
                    (message "%s" (datetime-read--preview-format final prior)))
                (error (message "%s" (error-message-string error)))))))

(defun datetime-read--preview-format (final prior)
    (concat
        (datetime-read--preview-format-1 'year  "4" final prior)
        "-"
        (datetime-read--preview-format-1 'month "2" final prior)
        "-"
        (datetime-read--preview-format-1 'day   "2" final prior)
        (if-let (final-value (decoded-time-weekday final))
            (let* ((names '(Sun Mon Tue Wed Thu Fri Sat))
                ...
                (if-let ((prior-value (decoded-time-weekday prior))
                    ...
                    (format " (%s) " name)))
            " ")
        (datetime-read--preview-format-1 'hour   "2" final prior)
        ":"
        (datetime-read--preview-format-1 'minute "2" final prior)
        ":"
        (datetime-read--preview-format-1 'second "2" final prior)))

(defun datetime-read--preview-format-1 (slot digits final prior)
    (let* ((format-string (concat "%0" digits "d"))
           (getter (intern (concat "decoded-time-" (symbol-name slot))))
           (final-value (funcall getter final))
           (prior-value (funcall getter prior)))
        (if (and prior-value (not (= prior-value final-value)))
            (format (concat "{" format-string "->" format-string "}")
