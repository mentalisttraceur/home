...
(defun make-binary-tree-array (length initial-value)
    (record 'binary-tree-array (make-vector length initial-value)))

(defun binary-tree-array-p (object)
    (eq (type-of object) 'binary-tree-array))

(defun binary-tree-array-vector (tree)
    (if (binary-tree-array-p tree)
        (aref tree 1)
        (if (vectorp tree)
            tree
            (signal 'wrong-type-argument (list 'binary-tree-array tree)))))

(defun binary-tree-array-size (tree)
    (length (binary-tree-array-vector tree)))

(defun binary-tree-array--bounds-check (index tree)
    (let ((size (binary-tree-array-size tree)))
        (unless (< -1 index size)
            (signal 'args-out-of-range (list tree index)))
        size))

(defun binary-tree-array--index--perfect-case (inorder-index height)
    (let* ((least-significant-zero (logffs (lognot inorder-index)))
           (level (- height least-significant-zero))
           (first-index-in-level (1- (ash 1 level)))
           (offset-in-level (ash inorder-index (- -1 least-significant-zero))))
        (+ first-index-in-level offset-in-level)))

(defun binary-tree-array--index (inorder-index size)
    (let* ((height       (logfls size))
           (lsb          (logand inorder-index 1))
           (even-index   (logxor inorder-index lsb))
           (vector-index (binary-tree-array--index--perfect-case
                             even-index height)))
        (if (< vector-index size)
            (if (equal lsb 0)
                vector-index
                (binary-tree-array--index--perfect-case
                    inorder-index height))
            (if (equal lsb 0)
                (let* ((prior-displaced-evens (- vector-index size))
                        (displacement (1+ (* 2 prior-displaced-evens))))
                    (binary-tree-array--index--perfect-case
                        (+ inorder-index displacement)
                        height))
                (let* ((prior-displaced-evens (1+ (- vector-index size)))
                        (displacement (* 2 prior-displaced-evens)))
                    (binary-tree-array--index--perfect-case
                        (+ inorder-index displacement)
                        height))))))

(defun binary-tree-array-ref (tree index)
    (let* ((size (binary-tree-array--bounds-check index tree))
           (vector-index (binary-tree-array--index index size)))
        (aref (binary-tree-array-vector tree) vector-index)))

(defun binary-tree-array-set (tree index value)
    (let* ((size (binary-tree-array--bounds-check index tree))
           (vector-index (binary-tree-array--index index size)))
        (aset (binary-tree-array-vector tree) vector-index value)))

(defun binary-tree-array (&rest objects)
    (let ((tree (make-binary-tree-array (length objects) nil))
          (index 0))
        (while objects
            (binary-tree-array-set tree index (pop objects))
            (setq index (+ index 1)))
        tree))
...
(defun make-histdir-history ()
    (let ((table      (make-sorted-hash-table 'string-greaterp :test 'equal))
        ...
        (vector table duplicates nil nil)))
(defun histdir-history-table (history)
    (aref history 0))
(defun histdir-history-duplicates (history)
    (aref history 1))
...
(defun histdir--update-buffer-local-history-pointers-1 (shared)
    (unless (dlist-car histdir-buffer-local-history--head)
        (setq histdir-buffer-local-history--head (dlist (dlist-car shared))))
    (dlist-setcdr histdir-buffer-local-history--head shared)
    (setq histdir-buffer-local-history-list
          (dlist-list histdir-buffer-local-history--head)))
...
(defun histdir-history--add (history key string)
    (let* ((table      (histdir-history-table history))
           (first      (aref table 3))
           (duplicates (histdir-history-duplicates history)))
        (when-let (replaced-string (dlist-car (bihash-get (aref table 2) key)))
            (unless (equal replaced-string string)
                (let ((keys (gethash replaced-string duplicates)))
                    (if-let (keys (delete key keys))
                        (puthash replaced-string keys duplicates)
                        (remhash replaced-string duplicates)))))
        (sorted-hash-table-put table key string)
        (unless (member key (gethash string duplicates))
            (push key (gethash string duplicates)))
        (unless (eq (aref table 3) first)
            (histdir--update-buffer-local-history-pointers history))))
(defun histdir-history--remove (history key)
    (let ((table      (histdir-history-table history))
          (duplicates (histdir-history-duplicates history)))
        (when-let ((first   (aref table 3))
                   (removed (bihash-get (aref table 2) key)))
            (let* ((string (sorted-hash-table-pop table key))
                   (keys   (gethash string duplicates)))
                (if-let (keys (delete key keys))
                    (puthash string keys duplicates)
                    (remhash string duplicates)))
        ...
        (with-temp-buffer
            (dolist (file files)
                ...
                (when-let (string (histdir--read-call (concat "call/" file)))
                    (histdir-history--add history file string))))))
(defun histdir--see-change (history watch-event)
    (let-unpack ((_descriptor action file) watch-event)
        (when (memq action '(deleted renamed))
            (let ((datetime (file-name-nondirectory file)))
...
(defun histdir-remove (entry)
    (let* ((hash (histdir--hash entry))
           ...
           (history          (gethash path histdir--histories))
           (duplicates (histdir-history-duplicates history))
           (datetimes (gethash entry duplicates))
...
(use-package evil
    ...
    (defun fixed-newline ()
        (interactive)
        (if (memq evil-state '(insert replace emacs))
...
(defun music-remove (prefix-argument)
    (interactive "P")
    (if prefix-argument
        (if (integerp prefix-argument)
            (if (< prefix-argument 0)
                (let* ((current (music-playlist-position))
                       (target (+ current prefix-argument)))
                    (when (< target 0)
                        (let ((count (music-playlist-count)))
                            (setq target (1- count))))
                    (music-playlist-remove target))
                (music-playlist-remove prefix-argument))
            (if (> (prefix-numeric-value prefix-argument) 4)
