...
(defun datetime-parse--loop (string &optional now)
    ...
    (let ((parsed (make-decoded-time))
        ...
        (unpack (parsed _ bindings)
                (datetime-parse--bind nil parsed integers t))
...
(defun datetime-parse--bind (slot parsed integers &optional mark-words)
    (setq parsed (copy-sequence parsed))
    (let ((bindings (make-decoded-time)))
