...
(defun make-histdir-history ()
    (let ((table (make-sorted-hash-table 'string-greaterp :test 'eq)))
...
(defun histdir--update-buffer-local-history-pointers (history)
    (let ((shared  (cddr (histdir-history-table history)))
    ...
    (let* ((table  (histdir-history-table history))
        ...
        (let ((new-last (dlist-cons string nil)))
            (if-let (last (histdir-history--last history))
                ...
                (setcdr (cdr table) new-last))
            (bihash-put bihash key new-last)
            (histdir-history--set-last history new-last))
        (when (= (bihash-count bihash) 1)
            (histdir--update-buffer-local-history-pointers history))))
(defun histdir-history--remove (history key)
    (let ((table (histdir-history-table history)))
        (when-let ((first   (cddr table))
                   (removed (bihash-get (car table) key)))
            (sorted-hash-table-pop table key)
        ...
        (with-temp-buffer
            (dolist (file files)
                ...
                (when-let (string (histdir--read-call (concat "call/" file)))
                    (let ((datetime (intern file)))
                        (histdir-history--add-oldest
...
(defun histdir--see-change (history watch-event)
    (let-unpack ((_descriptor action file) watch-event)
        (when (memq action '(deleted renamed))
            (let ((datetime (intern (file-name-nondirectory file))))
        ...
        (when (memq action '(created changed renamed))
            (let ((datetime (intern (file-name-nondirectory file)))
                ...
                (when (and string (> (length string) 0))
                    (histdir-history--add-newest history datetime string))))))
...
(use-package evil
    ...
    (evil-define-motion evil-find-char-pair-to-backward (count char1 char2)
        ...
        (evil-find-char-pair (- count) char1 char2)
        (forward-char 2))
    (define-key evil-motion-state-map "gf" 'evil-find-char-pair)
    (define-key evil-motion-state-map "gF" 'evil-find-char-pair-backward)
    (define-key evil-motion-state-map "gt" 'evil-find-char-pair-to)
    (define-key evil-motion-state-map "gT" 'evil-find-char-pair-to-backward)
    (when termux
...
(define-key music-map "p" 'music-pause-toggle)
