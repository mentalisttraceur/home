...
(use-package vertico
    ...
    (defun fixed-vertico-resize (&rest _)
        ...
        (let ((needed-height (count-vertico-candidate-lines))
            ...
            (unless (or (= vertico-count vertico-max-count)
                        (= needed-height vertico-max-height))
                (while (and (< needed-height vertico-max-height)
                            (< vertico-count vertico-max-count))
                    (setq vertico-count (1+ vertico-count))
                    (setq needed-height (fixed-vertico-resize--redisplay)))
                (when (and (> needed-height vertico-max-height)
                           (> vertico-count 1)
                           (>= (- vertico--index vertico--scroll)
                               (- vertico-count (1+ vertico-scroll-margin)))
                           (or (and (eq this-command 'vertico-next)
                                    (> vertico--index -1))
                               (and (eq this-command 'vertico-previous)
