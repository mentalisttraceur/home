...
(use-package evil
    ...
    (defun smoother-kill-buffer (&optional prefix-argument)
        (interactive "P")
        (if prefix-argument
            (let ((names (read-buffer-multiple "Kill buffers: "))
                  (count 0))
                (dolist (name names)
    ...
    (let ((count   (mpv-ipc-expand-integer socket "${playlist-count}"))
        ...
        (dotimes (index count)
            (let* ((path (mpv-ipc-expand socket
                   ...
                   (line (format "%d. %s\n" (1+ index) file)))
                (setq line (music--propertize line index path))
                (when (= index current)
...
(music-define-key "gp" 'music-replacing-paste-after)
(music-define-key "gP" 'music-replacing-paste-before)
(defun music-open-after (count)
    (interactive "p")
    (music--open count (music-select) (music--index-for-point 1)))
(music-define-key "o" 'music-open-after)
(defun music-open-before (count)
    (interactive "p")
    (music--open count (music-select) (music--index-for-point 0)))
(music-define-key "O" 'music-open-before)
...
(music-define-key "x" 'music-pause-toggle)
(defun music-pause ()
    (interactive)
    (let ((paused (mpv-ipc-set music--socket "pause" "yes")))
        (message "mpv pause: %s" paused)))
(music-define-key "X" 'music-pause)
...
(defun music-volume-up (count)
    ...
    (let ((volume (mpv-ipc-add music--socket "volume" count)))
        (message "mpv volume: %s" volume)))
(music-define-key "+" 'music-volume-up)
...
(defun music-seek-forward-second (count)
    (interactive "p")
    (mpv-ipc music--socket `(seek ,count)))
(defun music-seek-forward-second-10x (count)
    (interactive "p")
    (music-seek-forward-second (* count 10)))
(defun music-seek-forward-minute (count)
    (interactive "p")
    (music-seek-forward-second (* count 60)))
(defun music-seek-forward-minute-10x (count)
    (interactive "p")
    (music-seek-forward-second (* count 600)))
(defun music-seek-forward-decisecond (count)
    (interactive "p")
    (music-seek-forward-second (/ count 10.0)))
(defun music-seek-backward-second (count)
    (interactive "p")
    (music-seek-forward-second (- count)))
(defun music-seek-backward-second-10x (count)
    (interactive "p")
    (music-seek-forward-second-10x (- count)))
(defun music-seek-backward-minute (count)
    (interactive "p")
    (music-seek-forward-minute (- count)))
(defun music-seek-backward-minute-10x (count)
    (interactive "p")
    (music-seek-forward-minute-10x (- count)))
(defun music-seek-backward-decisecond (count)
    (interactive "p")
    (music-seek-forward-decisecond (- count)))
