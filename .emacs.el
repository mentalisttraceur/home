...
(defun datetime-parse--loop (string &optional now)
    ...
    (let ((parsed (make-decoded-time))
        ...
        (while word
            ...
            (unpack (parsed previous integers bindings)
                    (datetime-parse--1
                        word parsed previous integers bindings now))
            (pop word))
...
(defun datetime-parse--1 (word parsed previous integers bindings now)
    (cond
        ...
        ((string-match-p "^[-+][0-9]+mo" (car word))
            (let* ((months (string-to-number (car word)))
                ...
                (unpack (parsed integers bindings)
                        (datetime-parse--bind 'day parsed integers bindings))
                (if (eq (decoded-time-month bindings) t)
                    (setf (decoded-time-month bindings) nil)
                    (setf (decoded-time-month bindings) t))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (fixed-decoded-time-add parsed delta))))
        ((string-match-p "^[-+][0-9]+w" (car word))
            (let* ((days  (* 7 (string-to-number (car word))))
                ...
                (unpack (parsed integers bindings)
                        (datetime-parse--bind 'hour parsed integers bindings))
                (if (eq (decoded-time-day bindings) t)
                    (setf (decoded-time-day bindings) nil)
                    (setf (decoded-time-day bindings) t))
                (setq parsed (datetime-parse--future-bias nil parsed now))
        ...
        ((string-match-p "^[-+][0-9]+h" (car word))
            (let* ((hours (string-to-number (car word)))
                ...
                (unpack (parsed integers bindings)
                        (datetime-parse--bind 'minute parsed integers bindings))
                (if (eq (decoded-time-hour bindings) t)
                    (setf (decoded-time-hour bindings) nil)
                    (setf (decoded-time-hour bindings) t))
                (setq parsed (datetime-parse--future-bias nil parsed now))
        ...
        ((string-match-p "^[-+][0-9]+m" (car word))
            (let* ((minutes (string-to-number (car word)))
                ...
                (unpack (parsed integers bindings)
                        (datetime-parse--bind 'second parsed integers bindings))
                (if (eq (decoded-time-minute bindings) t)
                    (setf (decoded-time-minute bindings) nil)
                    (setf (decoded-time-minute bindings) t))
                (setq parsed (datetime-parse--future-bias nil parsed now))
        ...
        ((string-match-p "^[-+][0-9]+s\\($\\|[^t]\\)" (car word))
            (let* ((seconds (string-to-number (car word)))
                ...
                (unpack (parsed integers bindings)
                        (datetime-parse--bind nil parsed integers bindings))
                (if (eq (decoded-time-second bindings) t)
                    (setf (decoded-time-second bindings) nil)
                    (setf (decoded-time-second bindings) t))
                (setq parsed (datetime-parse--future-bias nil parsed now))
                (setq parsed (datetime-parse--floor nil parsed))
    ...
    (list parsed previous integers bindings))
