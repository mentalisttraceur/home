...
(use-package image-mode
    ...
    (defun image-scale-increase (&optional count position)
        (interactive "p\nd")
        (setq-if-nil count 1)
        (let* ((image (image--get-image position))
            ...
            new-scale))
    (defun image-scale-decrease (&optional count position)
        (interactive "p\nd")
        (setq-if-nil count 1)
        (image-scale-increase (- count) position))
    (defvar-local image-scale--initial nil)
    (defun image-scale-reset (&optional position)
        (interactive "d")
        (let ((image (image--get-image position)))
...
(use-packages doc-view
    :config
    (defun fixed-doc-view-enlarge (doc-view-enlarge factor)
        (let ((vscroll (window-vscroll nil t))
              (hscroll (window-hscroll))
              (height  (window-body-height nil t))
              (width   (window-body-width)))
            (prog1
                (funcall doc-view-enlarge factor)
                (let ((offset (/ (* height (- factor 1)) 2)))
                    (setq vscroll (round (+ (* vscroll factor) offset))))
                (let ((offset (/ (* width (- factor 1)) 2)))
    ...
    (advice-add 'doc-view-enlarge :around 'fixed-doc-view-enlarge)
    (define-plus-minus-equal-keys
