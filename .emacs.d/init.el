...
(defvar pixel-scroll-align-edge-incompatible-major-modes
    '(image-mode doc-view-mode))

(defun pixel-scroll-align-edge ()
    (unless (memq major-mode pixel-scroll-align-edge-incompatible-major-modes)
...
(defun interpreted-function-lisp (function)
    (let ((arglist  (aref function 0))
          (body     (aref function 1))
          (bindings (aref function 2)))
        (when (> (length function) 5)
            (when-let* ((interactive (aref function 5)))
                (if (vectorp interactive)
                    (let ((modes (aref interactive 1)))
                        (push (aref interactive 0) modes)
                        (push 'interactive modes)
                        (setq interactive modes))
                    (setq interactive (list 'interactive interactive)))
                (push interactive body)))
        (when (> (length function) 4)
            (when-let* ((docstring (aref function 4)))
                (push docstring body)))
        (push arglist body)
        (push 'lambda body)
            ...
            (let ((varlist ()))
                (dolist (binding bindings)
                    (let ((symbol (car binding))
                          (value  (cdr binding)))
                        (push (list symbol value) varlist)))
                (list 'let (nreverse varlist) body))
            body)))
...
(when android
    (defun define-translation (from-event to-event)
        (dolist (modifiers modifier-combinations)
            (define-key input-decode-map
                (vector (tool-bar-apply-modifiers from-event modifiers))
                (if (functionp to-event)
                    (if (memq to-event modifiers)
                        (intern (concat (symbol-name to-event) "-lock"))
                        to-event)
    ...
    (define-translation ?⁷ 'right)
    (define-translation ?⁸ 'deletechar)
    (define-translation ?⁹ 'meta)
    (define-translation ?⁰ 'escape)
    (define-translation ?⅝ 'home)
    (define-key global-map [M-down] [next])
    (define-key global-map [M-up] [prior])
...
(defun fixed-field-at-pos (&optional position)
    (setq-if-nil position (point))
    (let* ((beginning (field-beginning position))
           (end       (field-end       position))
           (field-1 (get-char-property beginning 'field))
           (field-2 (get-char-property position  'field)))
        (if (= end position)
...
(advice-add 'field-at-pos :override 'fixed-field-at-pos)

(defun fixed-field-string (&optional position)
    (multiple-values-bind (_ beginning end) (field-at-pos position)
...
(defun histdir-history--add-call (history string datetime)
    (let* ((calls (histdir-history-calls history))
           (datetimes (gethash string calls)))
        (unless (member datetime datetimes)
            (push datetime datetimes)
            (puthash string datetimes calls))))
(defun histdir-history--remove-call (history string datetime)
    (let* ((calls (histdir-history-calls history))
           (datetimes (gethash string calls)))
        (if-let* ((datetimes (delete datetime datetimes)))
            (puthash string datetimes calls)
            (remhash string calls))))
        ...
        (when-let* ((replaced-string (sorted-hash-table-get table datetime)))
            (unless (equal replaced-string string)
                (histdir-history--remove-call
                    history replaced-string datetime)))
        (sorted-hash-table-put table datetime string)
        (histdir-history--add-call history string datetime)
    ...
    (let ((table (histdir-history-table history)))
        (when-let* ((removed (bihash-get (aref table 2) datetime)))
            (let ((string (sorted-hash-table-pop table datetime)))
                (histdir-history--remove-call history string datetime))
...
(defun histdir--datetime-from-path (path)
    (let ((file (file-name-nondirectory path)))
        (when (string-match-p histdir--datetime-regex file)
            file)))
(defun histdir--read (histdir history)
    (let ((default-directory histdir)
          (files nil))
        (thread-yield)
        (condition-case _error
            (setq files (directory-files "." nil "..." t))
            (file-missing))
        (thread-yield)
        ...
        (with-temp-buffer
            (dolist (file files)
                (thread-yield)
                (histdir--read-1 file history)))))
(defun histdir--read-1 (file history)
    (when-let* ((datetime (histdir--datetime-from-path file))
                (string   (histdir--read-file file)))
        (when (length> string 0)
            (histdir-history--add history datetime string))))
(defun histdir--see-change (history watch-event)
    (seq-let (_descriptor action file) watch-event
        ...
        (when (memq action '(created changed renamed))
            (with-temp-buffer
                (histdir--read-1 file history)))))
(defun histdir--watch (directory history)
    (file-notify-add-watch directory '(change)
        (apply-partially 'histdir--see-change history)))
(defvar histdir)
...
(defun histdir-add (entry &optional deduplicate)
    (let ((datetime (format-time-string "%Y%m%dT%H%M%S,%NZ" nil t))
          (path     (expand-file-name histdir)))
        (histdir--init path)
        (with-temp-file (concat path "/" datetime)
            (insert entry "\n"))
        (when deduplicate
            (let* ((history (gethash path histdir--histories))
                   (calls (histdir-history-calls history))
                   (datetimes (gethash entry calls)))
                (dolist (other-datetime (remove datetime datetimes))
                    (delete-file (concat path "/" other-datetime)))))))
(defun histdir-remove (entry)
    (let* ((path (expand-file-name histdir))
        ...
        (dolist (datetime datetimes)
            (let ((file (concat path "/" datetime)))
                (delete-file file)))))
(defun histdir-change (old-entry new-entry)
    (let* ((path (expand-file-name histdir))
           (history (gethash path histdir--histories))
           (calls (histdir-history-calls history))
           (datetimes (gethash old-entry calls)))
        (dolist (datetime datetimes)
            (let ((file (concat path "/" datetime)))
                (with-temp-file file
                    (insert new-entry "\n"))))))
...
(defun smoother-eval-last-sexp (prefix-argument)
    (interactive "P")
    (unless (and (integerp temporary-goal-column)
...
(use-package dired
    ...
    (defun dired-binary-search (wanted &optional predicate)
        (setq-if-nil predicate 'string-lessp)
        (setq wanted (directory-file-name wanted))
        ...
        (let ((start (point))
            ...
            (while (< start end)
                (let* ((size (- end start))
                       (half (/ size 2)))
                    (goto-char (+ start half)))
                (let ((found (dired-get-filename nil t)))
                    (if (funcall predicate found wanted)
...
(use-package image-mode
    ...
    (defun fixed-image-rescale (function &rest arguments)
        (let* ((vscroll     (window-vscroll nil t))
               (hscroll     (window-hscroll))
               (view-height (window-body-height nil t))
               (view-width  (window-body-width))
               (image       (image--get-image))
               (old-height (cdr (image-size image t)))
               (old-width  (car (image-size image))))
            (prog1
                (apply function arguments)
                (setq image (image--get-image))
                (let* ((new-height (cdr (image-size image t)))
                       (factor (/ (float new-height) old-height))
                       (offset (/ (* view-height (- factor 1)) 2)))
                    (setq vscroll (round (+ (* vscroll factor) offset))))
                (let* ((new-width (car (image-size image)))
                       (factor (/ (float new-width) old-width))
                       (offset (/ (* view-width (- factor 1)) 2)))
                    (setq hscroll (round (+ (* hscroll factor) offset))))
                (image-bottom-edge)
                (unless (< (window-vscroll nil t) vscroll)
                    (image-set-window-vscroll vscroll))
                (image-right-edge)
                (unless (< (window-hscroll) hscroll)
                    (image-set-window-hscroll hscroll)))))
    (defvar image-scale-factor 1.25)
    (defun image-scale-increase (&optional count)
        (interactive "p")
        (setq-if-nil count 1)
        (let* ((image (image--get-image))
               (scale (plist-get (cdr image) :scale))
               (new-scale (* scale (expt image-scale-factor count))))
            (plist-put (cdr image) :scale new-scale)
            (force-window-update (current-buffer))
            new-scale))
    (advice-add 'image-scale-increase :around 'fixed-image-rescale)
    (defun image-scale-decrease (&optional count)
        (interactive "p")
        (setq-if-nil count 1)
        (image-scale-increase (- count)))
    (defvar-local image-scale--initial nil)
    (defun image-scale-reset ()
        (interactive)
        (let ((image (image--get-image)))
            (plist-put (cdr image) :scale image-scale--initial)
            (force-window-update (current-buffer))
            image-scale--initial))
    (advice-add 'image-scale-reset :around 'fixed-image-rescale)
    (defun image-scale--normalize (&optional position)
        (let* ((image (image--get-image position))
               (image-base (image--image-without-parameters image))
               (scale (image--current-scaling image image-base)))
            (plist-put (cdr image-base) :scale scale)
            (setcdr image (cdr image-base))
            scale))
...
(use-packages doc-view
    :config
    (advice-add 'doc-view-enlarge :around 'fixed-image-rescale)
    (define-plus-minus-equal-keys
...
(use-package hexl
    ...
    (defun fixed-hexlify-buffer--1 (function inverse-function)
        ...
        (when (listp buffer-undo-list)
            (push
                ...
                buffer-undo-list)))
    (defun fixed-hexlify-buffer (hexlify-buffer)
        (fixed-hexlify-buffer--1 hexlify-buffer 'dehexlify-buffer))
    (advice-add 'hexlify-buffer :around 'fixed-hexlify-buffer)
    (defun fixed-dehexlify-buffer (dehexlify-buffer)
        (fixed-hexlify-buffer--1 dehexlify-buffer 'hexlify-buffer))
    (advice-add 'dehexlify-buffer :around 'fixed-dehexlify-buffer)
    (defun fixed-hexl-mode (hexl-mode &optional argument)
        (let ((buffer-undo-list t))
            (funcall hexl-mode argument))
        (push
            `(apply fixed-hexl-mode--undo ,argument ,(point))
            buffer-undo-list))
    (defun fixed-hexl-mode--undo (argument point)
        (goto-char point)
        (hexl-mode-exit argument)
        (hack-local-variables))
    (advice-add 'hexl-mode :around 'fixed-hexl-mode)
    (defun fixed-hexl-mode-exit (hexl-mode-exit &optional argument)
        (let ((buffer-undo-list t))
            (funcall hexl-mode-exit argument))
        (push
            `(apply fixed-hexl-mode-exit--undo ,argument ,(point))
            buffer-undo-list))
    (defun fixed-hexl-mode-exit--undo (argument point)
        (goto-char point)
        (hexl-mode argument))
    (advice-add 'hexl-mode-exit :around 'fixed-hexl-mode-exit))
...
(defmacro datetime-parse--1 ()
    `(cond
         ((string-match-p "^[0-9]\\{3,\\}$" (car word))
             (datetime-parse--set year (car word)))
         ...
         ((string-match-p "^[0-9]+y" (car word))
             (let ((number (+ (string-to-number (car word))
                 ...
                 (datetime-parse--set year number)))
         ((string-match-p "^'[0-9]\\{1,3\\}$" (car word))
             (let ((number (+ (string-to-number (substring (car word) 1))
                              datetime-parse-two-digit-year-base)))
                 (datetime-parse--set year number)))
         ((string-match-p "^[0-9]+mo" (car word))
             (datetime-parse--set month (car word)))
         ((string-match-p "^[0-9]+\\(d\\|st\\|nd\\|rd\\|th\\)" (car word))
             (datetime-parse--set day (car word)))
         ((string-match-p "^[0-9]\\{3,\\}-[0-9][0-9]-[0-9][0-9]$" (car word))
             (seq-let (year month day) (string-split (car word) "-")
                 (datetime-parse--set year  year)
                 (datetime-parse--set month month)
                 (datetime-parse--set day   day)))
         ((string-match-p "^[0-9]\\{3,\\}-[0-9][0-9]$" (car word))
             (seq-let (year month) (string-split (car word) "-")
                 (datetime-parse--set year  year)
                 (datetime-parse--set month month)))
         ((string-match-p "^[0-9]+h" (car word))
             (datetime-parse--set hour (car word)))
         ((string-match-p "^[0-9]+am" (car word))
             (let ((number (string-to-number (car word))))
                 (when (= number 12)
                     (setq number 0))
                 (datetime-parse--set hour number)))
         ...
         ((string-match-p "^[0-9]+m" (car word))
             (datetime-parse--set minute (car word)))
         ((string-match-p "^[0-9]+s" (car word))
             (datetime-parse--set second (car word)))
         ((string-match-p "^[0-9][0-9]?:[0-9][0-9]:[0-9][0-9]$" (car word))
             (seq-let (hour minute second) (string-split (car word) ":")
                 (datetime-parse--set hour   hour)
                 (datetime-parse--set minute minute)
                 (datetime-parse--set second second)))
         ((string-match-p "^[0-9][0-9]?:[0-9][0-9]$" (car word))
             (seq-let (hour minute) (string-split (car word) ":")
                 (datetime-parse--set hour   hour)
                 (datetime-parse--set minute minute)))
         ((string-match-p "^[-+][0-9]+y" (car word))
         ...
         ((string-match-p "^ja" (car word))
             (datetime-parse--set month 1))
         ((string-match-p "^fe" (car word))
             (datetime-parse--set month 2))
         ((string-match-p "^\\(mr\\|mar\\)" (car word))
             (datetime-parse--set month 3))
         ((string-match-p "^ap" (car word))
             (datetime-parse--set month 4))
         ((string-match-p "^\\(my\\|may\\)" (car word))
             (datetime-parse--set month 5))
         ((string-match-p "^\\(jn\\|jun\\)" (car word))
             (datetime-parse--set month 6))
         ((string-match-p "^\\(jl\\|jul\\)" (car word))
             (datetime-parse--set month 7))
         ((string-match-p "^au" (car word))
             (datetime-parse--set month 8))
         ((string-match-p "^se" (car word))
             (datetime-parse--set month 9))
         ((string-match-p "^oc" (car word))
             (datetime-parse--set month 10))
         ((string-match-p "^\\(no$\\|nov\\)" (car word))
             (datetime-parse--set month 11))
         ((string-match-p "^de" (car word))
             (datetime-parse--set month 12))
         ((string-match-p "^now$" (car word))
             (datetime-parse--set year   now)
             (datetime-parse--set month  now)
             (datetime-parse--set day    now)
             (datetime-parse--set hour   now)
             (datetime-parse--set minute now)
             (datetime-parse--set second now))
...
(defmacro datetime-parse--set (slot value)
    (let* ((name (symbol-name slot))
           (decoded-time-slot (intern (concat "decoded-time-" name))))
        (cond
            ((eq value 'now)
                (setq value `(,decoded-time-slot ,value)))
            ((not (or (integerp value)
                      (eq value 'number)))
                (setq value `(string-to-number ,value))))
        (let ((form `((,decoded-time-slot parsed) ,value)))
            (unless (eq slot 'year)
                (push nil form)
                (push `(,decoded-time-slot bindings) form))
            (push 'setf form)
            form)))
...
(use-package vertico
    ...
    (defun fixed-vertico--last-visible-index ()
        (+ vertico--scroll vertico-count -1))
    (defvar-local fixed-vertico--offset nil)
    (defun fixed-vertico--offset ()
        (if (> fixed-vertico--scroll-direction 0)
            (- vertico--index (fixed-vertico--last-visible-index))
            (- vertico--index vertico--scroll)))
    (defvar vertico-max-height 10)
    (defvar vertico-max-count 10)
    (defvar-local fixed-vertico--scroll-direction -1)
    (defun fixed-vertico-resize--exhibit (&rest _)
        (let ((previous vertico--scroll))
            (vertico--compute-scroll)
            (let ((difference (- vertico--scroll previous)))
                (setq fixed-vertico--offset nil)
                (cond
                    ((/= difference 0)
                        (setq fixed-vertico--scroll-direction difference))
                    ((<= vertico--index vertico--scroll)
                        (setq fixed-vertico--scroll-direction -1))
                    ((>= vertico--index (fixed-vertico--last-visible-index))
                        (setq fixed-vertico--scroll-direction 1))
                    (t
                        (setq fixed-vertico--offset (fixed-vertico--offset))))))
        ...
        `(save-excursion
             ...
             (let ((end-of-input (point))
                 ...
                 (save-mutation
                     (insert lines)
                     ,@body))))
    (defun fixed-vertico-resize--chop-bottom ()
    ...
    (defun fixed-vertico-resize--align (bottom)
        ...
        (let* ((vertico-scroll-margin 0)
            ...
            (while (and (< needed-height vertico-max-height)
                        (< vertico-count vertico-max-count)
                        (< vertico-count vertico--total))
                    (+= vertico-count 1)
                    (if bottom
                        (if fixed-vertico--offset
                            (if (< fixed-vertico--offset 0)
                                (+= fixed-vertico--offset 1)
                                (-= vertico--scroll 1))
                            (-= vertico--scroll 1))
                        (when fixed-vertico--offset
                            (when (> fixed-vertico--offset 0)
                                (-= fixed-vertico--offset 1)
                                (-= vertico--scroll 1))))
                    (setq needed-height (fixed-vertico-resize--redisplay)))
            (when (> needed-height vertico-max-height)
                (if bottom
                    (fixed-vertico-resize--chop-top)
                    (fixed-vertico-resize--chop-bottom))
        ...
        (let ((point (point))
            ...
            (condition-case _error
                (while (> count 0)
                    ...
                    (when (equal (char-after (1+ (point))) char2)
                        (-= count 1)))
                (user-error
                    (goto-char point)
...
(use-packages doc-view evil
    :config
    (dolist (key '("h" [left]))
        (evil-define-key* 'motion doc-view-mode-map key 'image-backward-hscroll))
    (dolist (key '("j" [down]))
        (evil-define-key* 'motion doc-view-mode-map key
...
(defun eshell/r (&rest command)
    (let* ((program      (car command))
        ...
        (histdir-repl command histdir
            (append '("PAGER=cat" "PYTHON_BASIC_REPL=1")
                process-environment))))
...
(use-packages evil pop-to-command
    ...
    (defmacro git (&rest arguments)
        (let* ((command (cons "git" (mapcar 'symbol-name arguments)))
               (joined (string-join command "-"))
               (cleaned (replace-regexp-in-string "--+" "-" joined))
               (name (intern cleaned)))
            `(defun ,name (prefix-argument)
                 (interactive "P")
...
(use-package denote
    ...
    (defun denoted-tag-edit (path)
        (let* ((tags    (denoted-tag-get path))
               (options (denoted-tags-in-directory))
               (updated (denoted-tag-prompt options tags))
               (unique  (seq-uniq updated)))
            (prog1
                (denoted-tag-set path unique)
                (add-evil-repeat
                    (denoted-tag-edit--repeat
                        (seq-difference updated tags)
                        (seq-difference tags updated))))))
    (defun denoted-tag-edit--repeat (added deleted)
        (apply-partially 'denoted-try
            (lambda-let (added deleted) (path)
                (let* ((tags      (denoted-tag-get path))
                       (remaining (seq-difference tags deleted))
                       (merged    (append remaining added))
    ...
    (defun history-or-tag-execute-or-edit (prefix-argument)
        (interactive "P")
        (denoted-try 'denoted-tag-edit 'history-execute))
    (defun history-or-tag-remove (prefix-argument)
        (interactive "P")
        (denoted-try 'denoted-tag-remove 'history-remove))
    (define-key evil-motion-state-map "gh" 'history-or-tag-execute-or-edit)
...
(defun mpv-ipc--send (socket command)
...
(defun mpv-ipc (socket command)
    ...
    (let ((reply (mpv-ipc--receive socket)))
        ...
        (gethash "data" reply)))
(defun mpv-ipc-batch (socket commands)
    (let* ((values ())
           (errors ())
           (total (length commands))
           (progress (occasional-progress-counter
                         (format "mpv commands %%d/%d" total))))
        (dolist (command commands)
            (mpv-ipc--send socket command)
            (funcall progress))
        (message nil)
        (setq progress (occasional-progress-counter
                           (format "mpv replies %%d/%d" total)))
        (dotimes (index total)
            (let ((reply (mpv-ipc--receive socket)))
                (let ((ipc-error (gethash "error" reply)))
                    (if (equal ipc-error "success")
                        (push (gethash "data" reply) values)
                        (push (list index ipc-error) errors))))
            (funcall progress))
        (message nil)
        (when errors
            (signal 'mpv-ipc-error (nreverse errors)))
        (nreverse values)))
(defun mpv-ipc-disable-events (socket)
    (mpv-ipc--send socket '("disable_event" "all"))
    (let ((reply (mpv-ipc--receive socket)))
...
(defvar-local music--refresh-timer nil)
(defun music--refresh-cancel ()
    (condition-case nil
        (cancel-timer music--refresh-timer)
        (wrong-type-argument)))
...
(defun music ()
    ...
    (if-let* ((buffer  (get-buffer "*Music*"))
        ...
        (let* ((socket-path (mpv-ipc-socket-path-from-process process))
            ...
            (mpv-ipc-disable-events socket1)
            (mpv-ipc-disable-events socket2)
            (set-process-query-on-exit-flag socket1 nil)
            (set-process-query-on-exit-flag socket2 nil)
            (setq music--socket socket1)
            (setq music--refresh-socket socket2)
            (add-hook 'kill-buffer-hook
                (lambda-let (socket-path) ()
                    (music--refresh-cancel)
...
(defvar-local music--number-width nil)
(defun music--insert-playlist (socket)
    (let* ((count   (mpv-ipc-expand-integer socket "${playlist-count}"))
           ...
           (number-width (ceiling (log (1+ count) 10))))
        (setq music--number-width number-width)
        (dotimes (index count)
            (when (= index current)
                (evil-set-marker ?^))
            (music--insert-file-line socket index number-width)
            (when (= index current)
                (music--insert-playing-info socket index)))))
(defun music--insert-file-line (socket index number-width)
    (let* ((file-line (music--file-line socket index))
           (format (format "%%%dd. " number-width))
           (number (format format (1+ index))))
        (insert file-line)
        (let ((overlay (make-overlay (pos-bol 0) (pos-bol 1) nil t)))
            (overlay-put overlay 'evaporate t)
            (overlay-put overlay 'before-string number))))
...
(defun music--playing-line-fix-point (event)
    ...
    (let* ((position (event-start event))
        ...
        (goto-char (pos-bol 0))
        (move-to-column (max 0 (- column music--number-width 2)))))
...
(defun music-normal-state ()
    ...
    (evil-normal-state)
    (music--goto-current))
(defun music--pre-command-undo-boundary ()
    (undo-boundary))
(evil-define-command music-undo (count)
    :repeat abort
    (interactive "p")
    (evil-undo count))
(music-define-key 'normal "u" 'music-undo)
(evil-define-command music-redo (count)
    :repeat abort
    (interactive "p")
    (evil-redo count))
(music-define-key 'normal "U" 'music-redo)
(defmacro music--undo-as (function-call)
    (seq-let (function &rest arguments) function-call
        `(push
             (list 'apply ',function ,@arguments)
             buffer-undo-list)))
(defun music-index-at (position)
    (or (get-text-property position 'mpv-index)
        (music-playlist-count)))
(defun music--delete (start end type register yank-handler)
    (let ((evil-was-yanked-without-register nil))
        (evil-yank start end type register yank-handler))
    (let* ((paths   (full-path-property-split start end))
...
(defun music--add (paths count index column offset move)
...
(evil-define-command music-paste-before (count register)
    :suppress-operator t
    (interactive "p<x>")
    (music--paste count register 0 nil))
(music-define-key 'normal "P" 'music-paste-before)
(defun music--replace (inserted count replaced index column move)
    (let ((total (* count (length inserted)))
          (next-replaced replaced)
          (next-index index)
          (commands ()))
        (dotimes (_ count)
            (dolist (path inserted)
                (when next-replaced
                    (pop next-replaced)
                    (push (list "playlist-remove" next-index) commands)
                    (+= next-index 1))
                (push (list "loadfile" path "insert-at" next-index) commands)))
        (mpv-ipc-batch music--socket commands)
        (music--undo-as
            (music--replace-undo inserted count replaced index column move))
        (when move
            (+= index total)))
    (setq music--refresh-next-index index)
    (setq music--refresh-next-column column)
    (revert-buffer))
(defun music--replace-undo (inserted count replaced index column move)
    (let ((next-replaced replaced)
          (next-index    index)
          (commands      ()))
        (dotimes (_ count)
            (dolist (_ inserted)
                ...
                (+= next-index 1)
                (when next-replaced
                    (let ((path (pop next-replaced)))
                        (push
                            (list "loadfile" path "insert-at" next-index)
                            commands)))))
        (mpv-ipc-batch music--socket commands)
        (music--undo-as
            (music--replace inserted count replaced index column move)))
    (setq music--refresh-next-index index)
    (setq music--refresh-next-column column)
    (revert-buffer))
(defun music--replacing-paste (count register move)
    (let* ((pasted (music--paths-for-paste register))
           (start (point))
           (end   (pos-eol (* count (length pasted))))
           (replaced (full-path-property-split start end))
           (indexes  (text-property-values start end 'mpv-index))
           (index  (or (car indexes) 0))
           (column (current-column)))
        (music--replace pasted count replaced index column move)))
(evil-define-command music-replacing-paste-before (count register)
    :suppress-operator t
    (interactive "p<x>")
    (music--replacing-paste count register nil))
(evil-define-command music-replacing-paste-after (count register)
    :suppress-operator t
    (interactive "p<x>")
    (music--replacing-paste count register t))
(music-define-key 'normal "gp" 'music-replacing-paste-after)
(music-define-key 'normal "gP" 'music-replacing-paste-before)
(defun music--open (paths count offset move)
    (let ((index  (music-index-at (point)))
          (column (current-column)))
        (if (< (point) (point-max))
            (+= index offset)
            (setq offset 0))
...
(defun music--change (paths start end type register yank-handler)
    (let ((evil-was-yanked-without-register nil))
        (evil-yank start end type register yank-handler))
    (let* ((changed (full-path-property-split start end))
           (indexes (text-property-values start end 'mpv-index))
           (index  (or (car indexes) 0))
           (column (current-column)))
        (music--replace paths 1 changed index column nil)))
(evil-define-operator music-change (start end type register yank-handler)
    :move-point nil
    :type line
    (interactive "<R><x><y>")
    (let ((paths (music-read "Change to: ")))
        (music--change paths start end type register yank-handler)))
...
(evil-define-operator music-change-line (register yank-handler)
    ...
    (let ((paths (music-read "Change to: "))
          (start (pos-bol 1))
          (end   (pos-bol 2)))
        (music--change paths start end 'line register yank-handler)))
...
(defun music-pause ()
    ...
    (let ((paused (mpv-ipc-set music--socket "pause" "yes")))
        (message "mpv pause: %s" paused)))
(evil-define-motion music-goto-current ()
    :type line
    (let ((start (line-number-at-pos))
        ...
        (evil-line-move (- end start))))
(music-define-key 'motion "x" 'music-goto-current)
(music-define-key 'normal "x" 'music-goto-current)
(evil-define-motion music-goto-current-exclusive ()
    :type line
    (let* ((start (line-number-at-pos))
           (_     (evil-save-column
                      (music--goto-current)))
        ...
        (cond
            ...
            ((> start end)
                (evil-line-move 1)))))
(music-define-key 'motion "X" 'music-goto-current-exclusive)
(music-define-key 'normal "X" 'music-goto-current-exclusive)
...
(defun tumblr-pull ()
    (interactive)
    (let* ((url (evil-paste-to-string 1))
