...
(defun music--revert-buffer (&rest _)
    (setq music--want-full-refresh t))
(defun music--post-command-full-refresh ()
    (when music--want-full-refresh
        (setq music--need-full-refresh t)
        (setq music--want-full-refresh nil)))
...
(defun music ()
    ...
    (if-let* ((buffer  (get-buffer "*Music*"))
        ...
        (add-hook 'post-command-hook 'music--post-command-full-refresh nil t)
...
(evil-define-operator music-delete (start end type register yank-handler)
    ...
    (let* ((text    (evil-paste-to-string 1 register))
           ...
           (index   (car indexes))
           (paths   (full-path-property-split start end)))
...
(defun music--open (count paths index)
    ...
    (push
        ...
        buffer-undo-list)
    (* count (length paths)))
(defun music--undo-open (count paths index)
...
(evil-define-command music-replacing-paste-before (count register)
    :suppress-operator t
    (interactive "p<x>")
    (let* ((new-paths (music--paths-for-paste register))
           (start     (point))
           (end       (pos-eol (* count (length new-paths))))
           (old-paths (full-path-property-split start end))
        ...
        (let ((index (music--index-for-point 0)))
            (music--open count new-paths index))))
