...
(defun histdir-repl (command histdir)
    (let* ((program       (car command))
        ...
        (evil-local-set-key 'normal [deletechar]
            'histdir-repl-delete-char+force)
        (evil-local-set-key 'normal "X" 'histdir-repl-backspace-char)
        (evil-local-set-key 'normal "\C-?" 'histdir-repl-backspace-char+force)
