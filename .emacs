    ...
    (defun history-or-tag-execute-or-add (prefix-argument) (interactive "P")
        (if-let (file (buffer-file-name))
            (if (denote-file-is-note-p file)
                (call-interactively 'denote-keywords-add)
                (user-error "Buffer not visiting a Denote file"))
            (consult-history-execute prefix-argument)))
    (defun history-or-tag-remove (prefix-argument) (interactive "P")
        (if-let (file (buffer-file-name))
            (call-interactively 'denote-keywords-remove)
            (consult-history-remove prefix-argument)))
