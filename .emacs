...
(use-package evil
    ...
    (defun vi-color-minibuffer (color) (apply-partially (lambda (color)
          (when (minibufferp)
              (set-face-foreground 'minibuffer-prompt color)))
	  color))
    (add-hook 'evil-normal-state-entry-hook (vi-color-minibuffer "#FF0000"))
    (add-hook 'evil-operator-state-entry-hook (vi-color-minibuffer "#FF8000"))
    (add-hook 'evil-insert-state-entry-hook (vi-color-minibuffer "#00FF00"))
    (add-hook 'evil-replace-state-entry-hook (vi-color-minibuffer "#FFFF00"))
    (add-hook 'evil-visual-state-entry-hook (vi-color-minibuffer "#8080FF"))
    (add-hook 'evil-emacs-state-entry-hook (vi-color-minibuffer "#8000FF"))
