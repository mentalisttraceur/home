...
(use-package evil
    ...
    (define-key space-map "vA" (git-on-current-file add -p))
    (define-key space-map "vq" (git checkout -p))
    (define-key space-map "vQ" (git-on-current-file checkout -p))
    (define-key space-map "vw" (git reset -p))
    (define-key space-map "vW" (git-on-current-file reset -p))
    (define-key space-map "ve" (git stash -p))
    (define-key space-map "vE" (git-on-current-file stash -p))
...
(use-package denote
    ...
    (defun denote-keywords-add-path (path)
        (if (denote-file-has-identifier-p path)
            (let* ((title    (denote-extract-title-slug-from-path path))
                ...
                (denote-rename-file path title sorted))
            (user-error "File is not Denote-named")))
    (defun denote-keywords-remove-path (path)
        (if (denote-file-has-identifier-p path)
            (let* ((title     (denote-extract-title-slug-from-path path))
                   (keywords  (denote-extract-keywords-from-path path))
                   (deleted   (denote--keywords-delete-prompt keywords))
                   (remaining (seq-difference keywords deleted)))
                (denote-rename-file path title remaining))
            (user-error "File is not Denote-named")))
    (defun history-or-tag-execute-or-add (prefix-argument) (interactive "P")
        (if-let (path (buffer-file-name))
            (if (denote-file-is-note-p path)
                (become-command 'denote-keywords-add)
    ...
    (defun history-or-tag-remove (prefix-argument) (interactive "P")
        (if-let (path (buffer-file-name))
            (if (denote-file-is-note-p path)
                (become-command 'denote-keywords-remove)
