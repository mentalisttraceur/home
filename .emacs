...
(use-package ace-window
        ...
        (unwind-protect
            (progn
                (setq evil-aw-select-arguments
                    (list state-markers action next))
                ...
                (let* ((window-list  (aw-window-list))
                    ...
                    (if (< window-count 1)
                        (message "(aw-window-list): %s" window-list)
                        (if (< window-count 2)
                            (evil-aw-select--single-window (car window-list))
                            (aw-select "" (lambda (window)
                                (funcall action window)
                                (funcall next)))))))
            (setq override-evil-mode-line-tag nil)))
    (define-prefix-command 'space-o-single-window-map)
    (defun evil-aw-select--single-window (window)
        (aw--make-backgrounds (list window))
        (aw--lead-overlay "1" (cons (point) window))
        (set-transient-map space-o-single-window-map nil (lambda ()
            (aw--done)
            (setq override-evil-mode-line-tag nil))))
    ...
    (define-key space-o-single-window-map [escape] 'ignore)
    (define-key space-o-single-window-map "\C-g" 'ignore)
    (define-key space-o-single-window-map "1" (lambda () (interactive)
        (funcall (cadr evil-aw-select-arguments) (car (aw-window-list)))
        (funcall (caddr evil-aw-select-arguments))))
    (define-key space-o-single-window-map "?" (lambda () (interactive)
        (aw-show-dispatch-help)
        (apply 'evil-aw-select evil-aw-select-arguments)))
    (define-key space-o-single-window-map [t] (lambda () (interactive)
        (let ((key (substring (this-command-keys) -1)))
            (when (stringp key)
                (setq key (string-to-char key)))
            (let ((action (car (alist-get key aw-dispatch-alist))))
                (if action
                    (funcall action)
                    (when (characterp key)
                        (setq key (char-to-string key)))
                    (message "No such candidate: %s, hit ‘C-g’ to quit." key)
                    (apply 'evil-aw-select evil-aw-select-arguments))))))
