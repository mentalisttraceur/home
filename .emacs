...
(defun histdir-repl (command histdir)
    (let* ((program       (car command))
        ...
        (evil-local-set-key 'normal "h" 'histdir-repl-backward-char)
        (evil-local-set-key 'normal "l" 'histdir-repl-forward-char)
...
(defun histdir-repl-append () (interactive)
    (goto-char (eat-point))
    (histdir-repl-forward-char-in-input 1)
    (evil-insert-state))
...
(defun histdir-repl-end-of-line () (interactive)
    (if (histdir-repl-point-in-input-p)
        ...
        (end-of-line)))
(defun histdir-repl-forward-char (&optional count) (interactive "p")
    (if (not (histdir-repl-point-in-input-p))
        (forward-char count)
        (goto-char (eat-point))
        (histdir-repl-forward-char-in-input count)
        (sleep-for 0.04)))
(defun histdir-repl-backward-char (&optional count) (interactive "p")
    (if (not (histdir-repl-point-in-input-p))
        (backward-char count)
        (goto-char (eat-point))
