...
(use-package eat
    ...
    (defun pop-eat-window (command &optional id)
        (let* ((name      (string-join command " "))
               (name      (if id (concat name " <" id ">") name))
               (program   (car command))
...
(use-package evil
    ...
    (defun pop-evil-eat-window (command &optional id)
        (let* ((evil-insert-state-modes '(eat-mode))
               (eat-mode-hook (cons 'pop-evil-eat-window--keys eat-mode-hook)))
            (pop-eat-window command id)))
