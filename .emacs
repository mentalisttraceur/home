...
(use-package evil
    ...
    (defun evil-color-code-vi-state ()
        (if (minibufferp)
            ...
            (set-face-background 'mode-line (cond
                ...
                (t                       "#FFFFFF")))))
    (add-hook 'evil-normal-state-entry-hook   'evil-color-code-vi-state)
    (add-hook 'evil-operator-state-entry-hook 'evil-color-code-vi-state)
    (add-hook 'evil-insert-state-entry-hook   'evil-color-code-vi-state)
    (add-hook 'evil-replace-state-entry-hook  'evil-color-code-vi-state)
    (add-hook 'evil-visual-state-entry-hook   'evil-color-code-vi-state)
    (add-hook 'evil-emacs-state-entry-hook    'evil-color-code-vi-state)
    (add-hook 'post-command-hook (lambda ()
        (while-no-input (evil-color-code-vi-state))))
