...
(use-package eat
    ...
    (defun hack-eat--t-write (arguments)
        (setcar arguments (propertize (car arguments)
            'read-only t
            'field 'output
            'eat-output t
            'rear-nonsticky t))
    ...
    (defun eat-repl (command &optional context name)
        (setq name (pop-to-command-buffer-name "eat" command context name))
        (let ((program   (car command))
              (arguments (cdr command))
              (directory default-directory)
            ...
            (local-set-key "\C-m" 'eat-repl-send-input-line)
            (local-set-key "\t"   'eat-repl-send-input-tab)
            (local-set-key [up]   'eat-repl-send-up-arrow)
            (local-set-key [down] 'eat-repl-send-down-arrow)
            buffer))
    (defun eat-repl-not-in-input-p ()
        (text-property-any (point) (buffer-end 1) 'field 'output))
    (defun eat-repl-send-input-line () (interactive)
        (if (eat-repl-not-in-input-p)
            (message "not in input")
            (evil-end-undo-step)
            (evil-start-undo-step)
            (eat-repl-send-unsent-input "\n")))
    (defun eat-repl-send-input-tab () (interactive)
        (if (eat-repl-not-in-input-p)
            (message "not in input")
            (evil-end-undo-step)
            (evil-start-undo-step)
            (let* ((prompt-length (- (field-beginning) (car (eat--t-bol)))))
                (eat-repl-send-unsent-input "\t")
                (sleep-for 0.1)
                (let* ((start (+ (car (eat--t-bol)) prompt-length))
                       (end   (car (eat--t-eol)))
                       (inhibit-read-only t)
                       (inhibit-modification-hooks t))
                    (remove-text-properties start end '(field t read-only t))
                    (add-text-properties start end '(
                        modification-hooks    (eat-repl--reset-input)
                        insert-in-front-hooks (eat-repl--reset-input)))))))
    (defun eat-repl-send-unsent-input (suffix)
        (let* ((end   (field-end))
               (start end)
               (input nil)
               (inhibit-modification-hooks t))
            (while (not (get-text-property (1- start) 'eat-output))
                (setq start (1- start)))
            ...
            (eat--send-string
                ...
                (concat input suffix))))
    (defun eat-repl--reset-input (change-start _change-end)
        (let* ((inhibit-modification-hooks t)
               (inhibit-read-only t)
               (input-start (field-beginning))
               (point       (point))
               (input-end   (field-end))
               (input-up-to-change
                   (buffer-substring-no-properties input-start change-start))
               (input-starting-at-change
                   (buffer-substring-no-properties change-start input-end)))
            (eat--send-string
                (get-buffer-process (current-buffer))
                "\C-e\C-u")
            (sleep-for 0.1)
            (delete-field)
            (insert-before-markers input-up-to-change)
            (insert input-starting-at-change)
            (goto-char point)))
    (defun eat-repl-send-up-arrow () (interactive)
        (if (eat-repl-not-in-input-p)
            (message "not in input")
            (eat--send-string
                (get-buffer-process (current-buffer))
                "\e[A")))
    (defun eat-repl-send-down-arrow () (interactive)
        (if (eat-repl-not-in-input-p)
            (message "not in input")
            (eat--send-string
                (get-buffer-process (current-buffer))
