        ...
        (dolist (word words)
            (cond
                ...
                ((string-match-p "^fe" word)
                    (setq month 02))
                ((string-match-p "^mar" word)
                    (setq month 03))
                ((string-match-p "^ap" word)
                    (setq month 04))
                ((string-match-p "^may" word)
                    (setq month 05))
                ((string-match-p "^jun" word)
                    (setq month 06))
                ((string-match-p "^jul" word)
                    (setq month 07))
...
(use-package denote
    ...
    (defun note-list () (interactive)
        ...
        (revert-buffer)
        (note--filter-dired))
    (defun note-list-newest () (interactive)
        (note-list)
        (goto-char (point-max))
        (redisplay)
        (dired-previous-line 1))
    (defun note-list-oldest () (interactive)
        (note-list)
        (goto-char 1)
        (dired-next-line 1))
    (define-key space-map "M" 'note-list-newest)
    ...
    (defun task-schedule (timestamp)
        (let ((position (point-position-with-scroll))
            ...
            (with-advice ('denote-update-dired-buffers :override 'ignore)
                (dired-goto-file
