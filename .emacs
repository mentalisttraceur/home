...
(use-package ace-window
    ...
    (defun window-state-toggle-auto-balance ()
        ...
        (setq window-state--loop t))
    (defun window-state-bury-buffer ()
        (bury-buffer)
        (setq window-state--loop t))
    ...
    (defun window-state-bury-to-kill-buffer ()
        (if (> (length (get-buffer-window-list (window-buffer) nil t)) 1)
            ...
            (kill-buffer))
        (setq window-state--loop t))
    (defun window-state-kill-buffer ()
        (kill-buffer)
        (setq window-state--loop t))
    (defun window-state-forward-key (key)
        (list key `(lambda ()
            (setq window-state--loop nil)
            (setq unread-command-events '(,key)))))
    ...
    (setq aw-dispatch-alist `(
        ...
        (?b window-state-bury-buffer)
        (?B window-state-unbury-buffer)
