...
(use-package evil
    ...
    (define-key evil-motion-state-map "gG" 'evil-end-of-buffer)
...
(defun histdir-repl (command histdir)
    (let* ((program       (car command))
        ...
        (evil-local-set-key 'normal "x" 'histdir-repl-delete-char)
        (dolist (key '([delete] [deletechar]))
            (evil-local-set-key 'normal key 'histdir-repl-delete-char+force))
        (evil-local-set-key 'normal "X" 'histdir-repl-backspace-char)
...
(defun histdir-repl-end-of-input () (interactive)
    ...
    (sleep-for 0.04)
    (point))
(defun histdir-repl-input-substring (start end)
...
(defun histdir-repl-enter-input (offset)
    (let* ((point-in-buffer   (point))
        ...
        (histdir-repl-backward-char-in-input (length
            (histdir-repl-input-substring desired-point end)))
...
(defun histdir-repl-enter+delete-input (count)
    (let* ((point-in-buffer   (point))
        ...
        (if (<= start point-in-buffer end)
            (setq desired-point point-in-buffer)
            (setq desired-point point-in-terminal))
        (setq length (length (histdir-repl-input-substring desired-point end)))
        (when (> length count)
...
(defun histdir-repl-delete-char+force (&optional count) (interactive "p")
    (let ((deleted (histdir-repl-enter+delete-input count)))
        (when (equal deleted 0)
            (buffer-process-send-string "\e[3~"))
        deleted))
(defun histdir-repl-backspace-char (&optional count) (interactive "p")
    (let-unpack ((start point) (histdir-repl-enter-input 0))
        ...
        (histdir-repl-backspace-char-in-input count)))
(defun histdir-repl-backspace-char+force (&optional count) (interactive "p")
    (let-unpack ((start point) (histdir-repl-enter-input 0))
        (setq count (max 1 (min count (- point start))))
        (histdir-repl-backspace-char-in-input count)))
...
(defun histdir-repl-evil-replace-state () (interactive)
    (setq histdir-repl--evil-replace-edges (histdir-repl-enter-input 0))
    (evil-replace-state))
(defun histdir-repl-self-input+replace () (interactive)
    (let ((point (point))
          (end (caddr histdir-repl--evil-replace-edges)))
    ...
    (save-excursion
        (let* ((point-in-buffer   (point))
            ...
            (prog1
                ...
                (histdir-repl-backward-char-in-input (length
                    (histdir-repl-input-substring point-in-terminal end)))))))
