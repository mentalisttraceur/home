...
(use-package ace-window
    ...
    (defun select-ace-window-if-exists (key digit)
        (let ((window-count (length (aw-window-list))))
            (if (< window-count digit)
                (message "There are only %d windows" window-count)
                (setq unread-command-events (listify-key-sequence key))
                (evil-ace-select-window))))
    (defun define-ace-window-select-if-exists-key (digit)
        (let ((key (number-to-string digit)))
            (define-key space-map key `(lambda () (interactive)
                (select-ace-window-if-exists ,key ,digit)))))
    (define-ace-window-select-if-exists-key 1)
    (define-ace-window-select-if-exists-key 2)
    (define-ace-window-select-if-exists-key 3)
    (define-ace-window-select-if-exists-key 4)
    (define-ace-window-select-if-exists-key 5)
    (define-ace-window-select-if-exists-key 6)
    (define-ace-window-select-if-exists-key 7)
    (define-ace-window-select-if-exists-key 8)
    (define-ace-window-select-if-exists-key 9)
